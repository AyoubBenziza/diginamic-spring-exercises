...
public interface DepartmentRepository extends JpaRepository<Department, Integer> {
    ...
    @Query("SELECT c FROM City c JOIN c.department d WHERE d.code = :departmentId")
    Set<City> findCities(@Param("departmentId") String departmentId);

    @Query("SELECT c FROM City c JOIN c.department d WHERE d.code = :departmentId AND c.population BETWEEN :minPopulation AND :maxPopulation")
    ...
    @Query("SELECT c FROM City c JOIN c.department d WHERE d.code = :departmentId ORDER BY c.population DESC LIMIT :n")
